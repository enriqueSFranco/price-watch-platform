...
export class AmazonScraper implements IScraper<ProductType> {
  ...
  private async extractProductData(page: Page): Promise<ProductType | null> {
    ...
    try {
      const productData = await page.evaluate((selectors) => {
        ...
        const centerCol = mainContainer.querySelector(selectors.LAYOUT.CENTER_COLUMN);

        // --- Extracción del Nombre ---
        const titleFeature = centerCol?.querySelector('#title_feature_div');
        const titleSection = titleFeature?.querySelector('#titleSection');
        const productNameH1 = titleSection?.querySelector('#title');
        const productNameText =
          productNameH1?.querySelector('#productTitle')?.textContent.trim() || 'N/A';
        ...
        if (priceElement) {
          ...
          if (priceElement.classList.contains('a-price-whole')) {
            const fraction =
              centerCol?.querySelector('span.a-price-fraction')?.textContent?.trim() || '';
          ...
          if (corePriceDisplay) {
            const fallbackPriceEle =
              corePriceDisplay.querySelector('span.aok-offscreen') ||
            ...
            if (fallbackPriceEle) {
              ...
              if (fallbackPriceEle.classList.contains('a-price-whole')) {
                const fraction =
                  corePriceDisplay.querySelector('span.a-price-fraction')?.textContent?.trim() ||
                  '';
                priceText = priceText + fraction;
              }
              currentPrice = parsePriceLocal(priceText);
        ...
        const leftCol = mainContainer.querySelector(selectors.LAYOUT.LEFT_COLUMN);

        let imageUrl;
        const imgEle = leftCol?.querySelector(
  ...
  async run(productUrl: string): Promise<ProductType | null> {
    ...
    try {
      ...
      const continueButton = await page.$('form[action*="validateCaptcha"] button[type="submit"]');

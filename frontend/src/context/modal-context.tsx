...
export function ModalProvider({ children }: Readonly<{ children: React.ReactNode }>) {
	...
	return (
		<ModalConext.Provider value={modalState}>
			<ModalDispatchConext.Provider value={modalDispatch}>{children}</ModalDispatchConext.Provider>
		</ModalConext.Provider>
	);
}

// reducer
export type ModalAction = { type: 'OPEN_MODAL'; payload: Omit<ModalState, 'isOpen'> } | { type: 'CLOSE_MODAL' };

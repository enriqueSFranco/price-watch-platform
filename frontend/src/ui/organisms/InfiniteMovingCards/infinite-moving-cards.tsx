...
export function InfiniteMovingCards<T>({
  ...
  return (
    <div className={containerClass} role="region" ref={containerRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} data-direction={direction} data-speed={speed}>
        <ul className={clsx(styles.carouselList, styles.scrollerInner)} role="list" aria-live="off" ref={scrollerRef}>
          {items.map((item, index) => (
            <li key={`carousel-item-${index}`} role="listitem" className={styles.carouselList__item }>{renderItem(item, index)}</li>
          ))}
        </ul>
    </div>
  );
